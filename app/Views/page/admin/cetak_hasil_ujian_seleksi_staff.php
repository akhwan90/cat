	...
	<!-- <body onload="window.print()"> -->
		...
		<table class="table table-no-bordered">
			...
			<tr>
				...
				<td width="20%"><?=$peserta['posisi_saat_ini'];?></td>
			</tr>
			<tr>
				<td width="15%">Usia</td>
				<td width="1%">:</td>
		...
		<table class="table table-bordered">
			<thead>
				<tr>
					...
					<th width="15%" class="text-center">Nama Kompetensi</th>
					<th width="20%" class="text-center">Definisi</th>
			...
			<tbody>
				...
				foreach ($syarat_kompetensi['critical'] as $cv) {
					...
					$nama_aspek = $kompetensi_staff[$cv]['name'];
					$deskripsi_aspek = $kompetensi_staff[$cv]['deskripsi_aspek'];

					$nilai_to_deskripsi = '-';
					if (!empty($kompetensi_staff[$cv]['nilai_to_deskripsi'][$nilai_konversi])) {
						$nilai_to_deskripsi = $kompetensi_staff[$cv]['nilai_to_deskripsi'][$nilai_konversi];
					}
		...
		<!-- <p>
			<table class="table-bordered">
				<tr><td width="60%">Skor Keputusan</td><td width="60%"><?=$jml_nilai_konversi_keputusan;?></td></tr>
			</table>
		</p> -->
		<p>
			<table class="table-bordered">
		...
		<table class="table-bordered" style="margin-top: 20px">
			<tr><td colspan="3">CULTURE VALUE PROFILE</td></tr>
		<?php 
		if (!empty($nilai_c)) {
			foreach ($nilai_c as $vck => $vcv) {
				$level_color = 'red';
				if ($vcv['nilai'] >= 5) {
					$level_color = 'green';
				}
				echo '<tr>
						...
						<td width="25%"><div style="width: '.(($vcv['nilai']/10)*100).'%; height: 25px; vertical-align: middle; background: '.$level_color.'; color: #fff; font-weight: bold; text-align: right; padding-top: 20px; ">'.$vcv['nilai'].'&nbsp;&nbsp;</div></td>
		...
		<p>&nbsp;</p>
		<table class="table-bordered" style="margin-top: 20px">
			<tr><td colspan="3">LEADERSHIP TRAIT PROFILE</td></tr>
		<?php 
		if (!empty($nilai_d)) {
			foreach ($nilai_d as $vck => $vcv) {
				$level_color = 'red';
				if ($vcv['nilai'] >= 5) {
					$level_color = 'green';
				}
